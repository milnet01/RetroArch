...
**Cumulative:** 117 distinct fixes across 60 files. ~73 ✅ closed (S2/S3/S5/S11/S12 close + S1+S10 sweep close + cloud-sync OOM closes via Phase 1 cap; S6/S7/S8 closed from spec-needed cluster prior runs), 3 🚧 in-progress (atomic-save 4/7 sites, deref-before-check 11/30, plaintext-credentials chmod-only), 6 🔄 deferred (libretro-common vendored items + clang-analyzer FPs needing reproducer; core_updater_list.c:928 push_entry ownership-move FP from Bundle 28 triage; S4 awaiting maintainer build-matrix answer for qnx/vivante/xegl). Plus the `s3.c:1633-1640` heap-overflow item from indie-review C2 was verified resolved-stale (current code uses safe `%.*s` and writes the terminator to a freshly-malloc'd buffer, not `data->data`). Audit S9 closes as a project-policy suppression — see `docs/private/AUDIT-POLICY.md`.
...
✅ **Cluster closed 2026-04-27** — all twelve spec-needed items now have either a code fix (S1, S2, S3, S5, S6, S7, S8, S10, S11, S12) or a project-policy suppression (S9 — canonical free-on-fail-label pattern, suppressed in `aggregate.py` and documented in `docs/private/AUDIT-POLICY.md`). S4 alone remains 🔄 deferred pending a libretro upstream build-matrix answer (whether `qnx/vivante/xegl` context drivers are ever built without `HAVE_EGL`). Spec at [`docs/private/specs/2026-04-27-audit-spec-needed-cluster.md`](specs/2026-04-27-audit-spec-needed-cluster.md). External research verified Wayland `wl_touch` `id` is uncapped (per `wayland.xml` v10) and XInput `XUSER_MAX_COUNT = 4` is fixed across all XInput versions (per Microsoft Win32 docs).
