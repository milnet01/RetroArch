...
**Cumulative:** 111 distinct fixes across 55 files. ~67 ✅ closed (cloud-sync OOM closes via Phase 1 cap; S6/S7/S8 close from spec-needed cluster), 3 🚧 in-progress (atomic-save 4/7 sites, deref-before-check 11/30, plaintext-credentials chmod-only), 5 🔄 deferred (libretro-common vendored items + clang-analyzer FPs needing reproducer; one new entry: core_updater_list.c:928 push_entry ownership-move FP from Bundle 28 triage). Plus the `s3.c:1633-1640` heap-overflow item from indie-review C2 was verified resolved-stale (current code uses safe `%.*s` and writes the terminator to a freshly-malloc'd buffer, not `data->data`).
...
🚧 **Spec drafted 2026-04-27** — all twelve items below are now resolved (decision + test shape + external refs) in [`docs/private/specs/2026-04-27-audit-spec-needed-cluster.md`](specs/2026-04-27-audit-spec-needed-cluster.md). Recommended implementation order is in that doc's first table; cluster moves from 📋 spec-needed → 🚧 spec-drafted-pending-impl. External research verified Wayland `wl_touch` `id` is uncapped (per `wayland.xml` v10) and XInput `XUSER_MAX_COUNT = 4` is fixed across all XInput versions (per Microsoft Win32 docs).
