...
**Cumulative:** 118 distinct fixes across 61 files. ~74 ✅ closed (full spec-needed cluster S1–S12 closes; cloud-sync OOM via Phase 1 cap; runahead state-size mid-run growth; …), 3 🚧 in-progress (atomic-save 4/7 sites, deref-before-check 11/30, plaintext-credentials chmod-only), 5 🔄 deferred (libretro-common vendored items + clang-analyzer FPs needing reproducer; core_updater_list.c:928 push_entry ownership-move FP from Bundle 28 triage). Plus the `s3.c:1633-1640` heap-overflow item from indie-review C2 was verified resolved-stale (current code uses safe `%.*s` and writes the terminator to a freshly-malloc'd buffer, not `data->data`). Audit S9 closes as a project-policy suppression — see `docs/private/AUDIT-POLICY.md`.
...
- ✅ **S2 — `MENU_LIST_GET` / `MENU_LIST_GET_SELECTION` macro contract.** `menu/menu_driver.h:72-78` plus 16 unguarded callers across `menu/cbs/menu_cbs_right.c:223`, `menu_cbs_sublabel.c:1946`, ozone (6 sites), xmb (8 sites). _(Fixed `335bb31cf9` Bundle 34 — option A: macro stays NULL-safe (returns NULL when `list` is NULL), all unguarded `->size` derefs swept to use `menu_list ? ...->size : 0` (matches the existing guard pattern at the ~half of sites that already had it). Where the site also assigned `selection_buf = MENU_LIST_GET_SELECTION(...)`, lifted that assignment above the size deref and used the local pointer for the guard. Plus a contract comment near the macro definition naming the audit/indie-review evidence (clang-analyzer C1 path-sensitive proof) so future maintainers don't re-introduce the unguarded pattern.)_
