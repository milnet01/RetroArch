...
- 🚧 **MEDIUM — `s3_update` reads entire file into RAM.** `s3.c:1838`. 5GB save state on a console with 256MB → OOM. Even on desktop: GB-scale uploads block the loop. Stream or cap. _(Spec drafted 2026-04-27 → [`docs/private/specs/2026-04-27-cloud-sync-streaming-upload-design.md`](specs/2026-04-27-cloud-sync-streaming-upload-design.md). Phase 1 = 2 GB cap (settings-configurable), applied to s3 + webdav + google_drive at upload entry. Phases 2-4 = streaming SHA-256 + pull-callback HTTP body + multipart audit. Phase 1 alone closes the indie-review concern; ~2 days for the full streaming refactor.)_
